...
> 채용공고 분석 · 이력서 매칭 · 자소서 생성 자동화 서비스 (v2)
...
> 기업마다 다른 자소서 양식에 맞게 항목을 직접 입력할 수 있습니다. "성장과정"을 추가하여 4개 항목으로 설정한 결과입니다.
...
v1에서는 FastAPI 백엔드 안에 LangGraph로 에이전트 그래프를 설계했습니다. 크롤링 → 분석 → 매칭 → 자소서 생성 흐름을 노드와 엣지로 연결하는 구조였는데, Streamlit Cloud 배포 시 FastAPI 서버를 별도로 띄울 수 없는 문제가 있었습니다. 배포 가능한 구조를 우선해서 FastAPI와 LangGraph를 제거하고 Streamlit 단독 구조로 전환했습니다. 덕분에 배포는 해결됐지만 에이전트 구조는 사라졌고, 이 경험이 이후 에이전트를 제대로 다시 설계해보고 싶다는 동기가 됐습니다.
...
*다음 버전: [Job Agent v3](https://github.com/HyeonBin0118/job-agent-v3)*
